...
- **能力边界**：定位为「合规留痕 + 风险提示」——事件完整性依赖 Agent 主动申报，
  无 L2/L3 阻断，不宣称安全防护；报告「申报完整性核对」小节会如实标注
  未闭合会话、静默区间与覆盖置信度，但该置信度**仅反映申报侧完整性，不代表行为覆盖**；
  进程交叉校验（T3.1，默认关闭）为会话 start/end 边界快照比对，仅输出「疑似二级
  操作」告警（不拦截），窗口内瞬时进程不可见、快照失败如实标注「不可用」；
  文件快照（T3.2）与 Windows 审计日志（T3.3）交叉校验同为边界窗口比对、仅告警
  不拦截（默认关闭），审计未启用时如实标注「不可用」并输出启用指引
...
以下差异随开发计划里程碑同步登记，**只列已验证项**（验证依据见
`../WorkBuddy降级监测适配/端到端验证结果与差距清单.md` 与《WorkBuddy Hook 支持性验证报告.md》）：

| 项 | 状态 | 说明 |
|---|---|---|
| 提示词 v2.1（红线条款 + 申报引导） | ✅ 已落地 | 声明参数脱敏符合工具内置约定；写/敏感操作前补 pre；绕过申报通道执行操作视为高风险并如实留痕（2026-09-03 实测不被判定为提示注入） |
| instructions 自动注入（T1.2） | ✅ 已落地 | `configure-workbuddy` 将提示词写入 `mcp.json` `instructions` 字段，新会话不手动粘贴即生效（2026-09-03 真实会话实测自发 pre/post 申报）；WorkBuddy 不消费该字段时回退手动粘贴路径 |
| 连接器健康看门狗（T1.3） | ✅ 已落地 | `preflight` 子命令检查 mcp.json 注册 + 端口 + 三申报工具就绪，失连时输出分步指引；daemon 侧申报静默超阈值仅日志提示，不判定不干预 |
| 申报完整性核对与覆盖置信度（T1.4） | ✅ 已落地 | 报告标注未闭合会话/仅结束未开始/静默区间 + 覆盖置信度（高/中/低）；`jsonl_dir` 未配置时如实标注「完整性核对跳过」 |
| 进程快照交叉校验（T3.1） | ✅ 已落地 | 会话 start/end 边界 psutil 快照差异与申报记录比对，申报外新增进程 → 「疑似二级操作」告警（仅告警不拦截）；系统/Agent 进程白名单防误报；留痕 `crosscheck_process.jsonl`；默认关闭，`workbuddy_connect.yaml` 的 `observer.crosscheck_process.enabled` 启用（2026-09-03 单测/e2e/真实环境 smoke 验证，M2 全量回归 1004 passed 无回归） |
| 文件快照交叉校验（T3.2） | ✅ 已落地 | 会话边界对受保护目录哈希快照比对，申报外变更 → 「疑似二级操作」告警（仅告警不拦截）；申报内变更不误报；留痕 `crosscheck_file.jsonl`；默认关闭，`workbuddy_connect.yaml` 的 `observer.crosscheck_file.enabled` 启用（2026-09-03 单测/e2e 验证，M3 全量回归无回归） |
| Windows 审计日志交叉校验（T3.3） | ✅ 已落地 | PowerShell Get-WinEvent 查询 4688/4104（系统日志与申报时间统一转 UTC 毫秒对齐），与申报命令/路径比对，申报外系统事件 → 「疑似二级操作」告警（仅告警不拦截）；审计未启用/无权限时如实标注「不可用」并输出启用指引；留痕 `crosscheck_audit.jsonl`；默认关闭，`workbuddy_connect.yaml` 的 `observer.crosscheck_audit.enabled` 启用（2026-09-03 单测 32 项 + e2e 全过，M3 全量回归 1071 passed 无回归） |
| MCP Hook 事前拦截（第 2 层） | ❌ 关闭 | T2.0 实测（2026-09-03）：WorkBuddy 无 PreToolUse/PostToolUse 注册与执行机制（UI 无入口 + mcp.json 无 hooks 字段 + 产物静态分析无运行时执行器），事前拦截不可承载；未来版本引入 Hook 后复测再启 |
| 跨会话关联（Q1）/ 时序聚合（Q2） | ❌ 未覆盖 | 需可信时间戳与长周期数据，独立立项，不在当前计划 |
