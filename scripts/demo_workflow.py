...
DEMO_DIR = os.path.join(PROJECT_ROOT, "deep-agents-demo")
...
def phase_2_start_monitor(env: dict):
    ...
    cmd_box("终端 1 — 启动监控 (Simulation 模式)", [
        ...
        "python3 scripts/deep_agent_monitor.py --scenario da04_production_demo --record",
    ], note="da04 是生产演示场景，--record 启用事件录制")

    print(f"\n  {BOLD}方式 B: 使用 test.py 菜单{RESET}")
    print(f"  在终端 1 中执行:\n")
    cmd_box("终端 1 — scripts/test.py 交互式", [
        "cd ~/projects/observer_sim",
    ...
    if env["api_key"] and env["pydantic_deep"]:
        ...
        cmd_box("终端 1 — Live 监控", [
            "cd ~/projects/observer_sim",
            "python3 scripts/deep_agent_monitor.py --live --record "
